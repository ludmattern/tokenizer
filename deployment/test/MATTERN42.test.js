...
describe('MATTERN42Token', function () {
	...
	beforeEach(async function () {
		...
		token = await MATTERN42Token.deploy('MATTERN42Token', 'M42T', ethers.parseEther('100000'));
	});
	...
	describe('Burning', function () {
		it('Should allow token holders to burn their tokens', async function () {
			...
			expect(await token.totalSupply()).to.equal(ethers.parseEther('99000'));
		});
	});
	...
	describe('Pausing', function () {
		...
		it('Should prevent transfers when paused', async function () {
			...
			await expect(token.transfer(addr1.address, ethers.parseEther('100'))).to.be.revertedWith('Pausable: paused');
		});
	});
});
